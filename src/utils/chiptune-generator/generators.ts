...
function getChord(
  { progression, key, scale }: MusicContext,
  rowIndex: number
): number[] {
  const progIndex = Math.floor(rowIndex / 4);
  const chordNumber = progression[progIndex];
  return music.chordTypes.triad.map(
    (noteIndex) => key + scale[(chordNumber - 1 + noteIndex) % scale.length]
  );
}
...
function arp(context: MusicContext): Pattern<Note> {
  ...
  return fill(PatternSize, (i) => {
    ...
    return {
      note:
        chord[(i + offset + choose([0, 0, 0, 1, 2])) % chord.length] +
        octave +
        choose([0, 12]),
      fx: {
...
function bass2(context: MusicContext): Pattern<Note> {
  return fill(PatternSize, (i) => {
    const chord = getChord(context, i);
    return {
      note: i % 8 === 0 ? ((chord[0] + 4) % 12) - 4 : "cont",
      vel: 2,
      fx: { pulseWidth: rnd() },
    } as Note;
  });
...
function melody1(context: MusicContext): Pattern<Note> {
  ...
  const pattern: Note[] = [];
  let current =
    choose(music.chordTypes.triad) -
    1 +
    context.scale.length * choose([2, 3, 4]);
  for (let i = 0; i < PatternSize; i++) {
    ...
    if ((slow && i % 2 === 1) || flip(0.1 + 0.4 * (1 - (i % 2)))) {
      pattern.push({
        note: "cont",
        fx: { pulseWidth: pwmMod ? pwmAmount : 0 },
      });
    } else {
      if (current > 10 && flip()) {
        current--;
      ...
      pattern.push({
        note:
          context.key +
          context.scale[current % context.scale.length] +
          Math.floor(current / context.scale.length) * 12,
        fx: {
...
function emptyNote(): Pattern<Note> {
  return fill(PatternSize, (_) => ({ note: "---" } as Note));
}
function emptyDrum(): Pattern<Drum> {
  return fill(PatternSize, (_) => ({ drum: "---" } as Drum));
}
function drum(): Pattern<Drum> {
  return fill(PatternSize, (i) => ({
    drum:
      i % 8 === 0
        ? "KCK"
